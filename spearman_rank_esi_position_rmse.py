#!/usr/bin/env python3
"""
Read ESI and Position RMSE values from spearman_rank_test_data_esi_vs_rmse and compute Spearman rank correlation.
"""
...
DATA_DIR = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'spearman_rank_test_data_esi_vs_rmse')
...
def main():
    ...
    with open(RMSE_PATH) as f:
        position_rmse = np.array([float(line.strip()) for line in f if line.strip()])

    if len(esi) != len(position_rmse):
        print(f"Error: Length mismatch: esi={len(esi)}, position_rmse={len(position_rmse)}")
        sys.exit(1)
    if len(esi) < 2:
    ...
    if HAS_SCIPY:
        res = scipy_spearmanr(esi, position_rmse)
        rho = getattr(res, 'statistic', res[0])
        p_value = getattr(res, 'pvalue', res[1])
    else:
        rho = spearmanr_numpy(esi, position_rmse)
        p_value = None

    print("Spearman rank correlation (ESI vs Position RMSE)")
    print("=" * 50)
