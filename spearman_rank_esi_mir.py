#!/usr/bin/env python3
"""
Read ESI and MIR (Map Integrity Ratio) values from spearman_rank_test_data_esi_vs_mir and compute Spearman rank correlation.
"""
...
DATA_DIR = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'spearman_rank_test_data_esi_vs_mir')
ESI_PATH = os.path.join(DATA_DIR, 'esi.txt')
MIR_PATH = os.path.join(DATA_DIR, 'mir.txt')
...
def main():
    ...
    if not os.path.exists(MIR_PATH):
        print(f"Error: File not found: {MIR_PATH}")
        sys.exit(1)

    with open(ESI_PATH) as f:
        esi = np.array([float(line.strip()) for line in f if line.strip()])
    with open(MIR_PATH) as f:
        mir = np.array([float(line.strip()) for line in f if line.strip()])

    if len(esi) != len(mir):
        print(f"Error: Length mismatch: esi={len(esi)}, mir={len(mir)}")
        sys.exit(1)
    if len(esi) < 2:
    ...
    if HAS_SCIPY:
        res = scipy_spearmanr(esi, mir)
        rho = getattr(res, 'statistic', res[0])
        p_value = getattr(res, 'pvalue', res[1])
    else:
        rho = spearmanr_numpy(esi, mir)
        p_value = None

    print("Spearman rank correlation (ESI vs MIR)")
    print("=" * 50)
