#!/usr/bin/env python3
"""
Read ESI and Yaw RMSE values from spearman_rank_test_data_esi_vs_yaw_rmse and compute Spearman rank correlation.
"""
...
DATA_DIR = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'spearman_rank_test_data_esi_vs_yaw_rmse')
ESI_PATH = os.path.join(DATA_DIR, 'esi.txt')
YAW_RMSE_PATH = os.path.join(DATA_DIR, 'yaw_rmse.txt')
...
def main():
    if not os.path.exists(ESI_PATH):
        ...
        sys.exit(1)
    if not os.path.exists(YAW_RMSE_PATH):
        print(f"Error: File not found: {YAW_RMSE_PATH}")
        sys.exit(1)

    with open(ESI_PATH) as f:
        esi = np.array([float(line.strip()) for line in f if line.strip()])
    with open(YAW_RMSE_PATH) as f:
        yaw_rmse = np.array([float(line.strip()) for line in f if line.strip()])

    if len(esi) != len(yaw_rmse):
        print(f"Error: Length mismatch: esi={len(esi)}, yaw_rmse={len(yaw_rmse)}")
        sys.exit(1)
    if len(esi) < 2:
    ...
    if HAS_SCIPY:
        res = scipy_spearmanr(esi, yaw_rmse)
        rho = getattr(res, 'statistic', res[0])
        p_value = getattr(res, 'pvalue', res[1])
    else:
        rho = spearmanr_numpy(esi, yaw_rmse)
        p_value = None

    print("Spearman rank correlation (ESI vs Yaw RMSE)")
    print("=" * 50)
